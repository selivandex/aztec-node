...
LOG_FILE="${LOGS_DIR}/check_sync_status_${INVENTORY_NAME}_$(date +%Y%m%d_%H%M%S).log"
...
log "Starting sync status check on all servers"
...
show_usage() {
    ...
    echo ""
    echo "This script checks sync status on all servers."
    echo "Runs the Cerberus sync-check script and collects results."
    echo ""
    echo "The script will show:"
    echo "  - Local block number"
    echo "  - Remote block number"
    echo "  - Sync progress percentage"
    echo "  - Sync status (synced/not synced)"
    echo ""
    echo "Examples:"
...
run_sync_check() {
    ...
    log "Checking sync status on all servers..."
    ...
    local ansible_cmd="ansible-playbook check_sync_status.yml -i $INVENTORY_PATH"
    ...
    if eval "$ansible_cmd"; then
        success "=== Sync status check completed successfully! ==="
    else
...
main() {
    ...
    log "Sync status has been checked on all servers"
