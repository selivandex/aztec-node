...
LOG_FILE="${LOGS_DIR}/aztec_restart_${INVENTORY_NAME}_$(date +%Y%m%d_%H%M%S).log"
...
log "Starting Aztec service restart process"
...
show_usage() {
    ...
    echo ""
    echo "This script restarts aztec-node.service on all servers in the specified inventory."
    echo ""
    echo "Examples:"
...
# Main function
main() {
    log "=== Aztec Service Restart Script ==="
    ...
    echo ""
    warning "This will restart aztec-node.service on ALL servers in inventory: $INVENTORY_NAME"
    warning "The service will be temporarily unavailable during restart."
    echo ""
    ...
    log "Starting Aztec service restart on all servers from inventory: $INVENTORY_NAME"
    log "This process should take 1-2 minutes depending on server count"
    ...
    local ansible_cmd="ansible-playbook restart_aztec.yml -i $INVENTORY_PATH --ssh-common-args='-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=10'"
    ...
    if eval "$ansible_cmd"; then
        success "=== Aztec service restart completed successfully! ==="
        log "All servers in inventory '$INVENTORY_NAME' have restarted aztec-node.service"
        ...
        error "=== Aztec service restart failed! ==="
        error "Some servers may have failed to restart the service"
...
cleanup() {
    if [ $? -ne 0 ]; then
        error "Restart script failed. Check log file: $LOG_FILE"
        error ""
        error "If some servers failed to restart, you can:"
        error "1. Check service status: sudo systemctl status aztec-node.service"
        error "2. Restart service manually: sudo systemctl restart aztec-node.service"
