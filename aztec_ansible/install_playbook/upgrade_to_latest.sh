...
LOG_FILE="${LOGS_DIR}/upgrade_to_latest_${INVENTORY_NAME}_$(date +%Y%m%d_%H%M%S).log"
...
log "Starting Aztec node upgrade to latest version"
...
show_usage() {
    ...
    echo ""
    echo "This script upgrades Aztec nodes to latest version by updating .aztec-run file."
...
run_upgrade() {
    log "=== Starting Aztec node upgrade to latest version ==="
    log "Updating .aztec-run files on all servers..."
    ...
    local ansible_cmd="ansible-playbook upgrade_to_latest.yml -i $INVENTORY_PATH"
    ...
    if eval "$ansible_cmd"; then
        success "=== Aztec node upgrade to latest completed successfully! ==="
    else
        error "=== Aztec node upgrade to latest failed! ==="
...
# Main function
main() {
    log "=== Aztec Node Upgrade to Latest Script ==="
    ...
    log "All Aztec nodes have been upgraded to latest version"
