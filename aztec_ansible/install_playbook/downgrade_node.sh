...
LOG_FILE="${LOGS_DIR}/downgrade_node_${INVENTORY_NAME}_$(date +%Y%m%d_%H%M%S).log"
...
log "Starting Aztec node downgrade to version 0.87.9"
...
show_usage() {
    ...
    echo ""
    echo "This script downgrades Aztec nodes to version 0.87.9."
...
run_downgrade() {
    log "=== Starting Aztec node downgrade to version 0.87.9 ==="
    log "Downgrading nodes on all servers..."
    ...
    local ansible_cmd="ansible-playbook downgrade_node.yml -i $INVENTORY_PATH"
    ...
    if eval "$ansible_cmd"; then
        success "=== Aztec node downgrade completed successfully! ==="
    else
        error "=== Aztec node downgrade failed! ==="
...
# Main function
main() {
    log "=== Aztec Node Downgrade Script ==="
    ...
    log "All Aztec nodes have been downgraded to version 0.87.9"
