...
LOG_FILE="${LOGS_DIR}/fix_testnet_${INVENTORY_NAME}_$(date +%Y%m%d_%H%M%S).log"
...
log "Starting testnet parameter fix in start_aztec_node.sh"
...
show_usage() {
    ...
    echo ""
    echo "This script fixes testnet parameter in start_aztec_node.sh file."
    echo "Changes 'alpha-testnet' to 'testnet' on all servers."
...
# Run fix
run_fix() {
    log "=== Starting testnet parameter fix ==="
    ...
    local ansible_cmd="ansible-playbook fix_testnet.yml -i $INVENTORY_PATH"
    ...
    if eval "$ansible_cmd"; then
        success "=== Testnet parameter fix completed successfully! ==="
    else
        error "=== Testnet parameter fix failed! ==="
...
# Main function
main() {
    log "=== Testnet Parameter Fix Script ==="
    ...
    log "Testnet parameter has been fixed in start_aztec_node.sh on all servers"
    log "Changed 'alpha-testnet' to 'testnet'"
