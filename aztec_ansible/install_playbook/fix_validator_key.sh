...
LOG_FILE="${LOGS_DIR}/fix_validator_key_${INVENTORY_NAME}_$(date +%Y%m%d_%H%M%S).log"
...
log "Starting validator key parameter fix in start_aztec_node.sh"
...
show_usage() {
    ...
    echo ""
    echo "This script fixes validator key parameter in start_aztec_node.sh file."
    echo "Changes 'validatorPrivateKey' to 'validatorPrivateKeys' on all servers."
...
# Run fix
run_fix() {
    log "=== Starting validator key parameter fix ==="
    log "Fixing start_aztec_node.sh files on all servers..."
    ...
    local ansible_cmd="ansible-playbook fix_validator_key.yml -i $INVENTORY_PATH"
    ...
    if eval "$ansible_cmd"; then
        success "=== Validator key parameter fix completed successfully! ==="
    else
        error "=== Validator key parameter fix failed! ==="
...
# Main function
main() {
    log "=== Validator Key Parameter Fix Script ==="
    ...
    log "Validator key parameter has been fixed in start_aztec_node.sh on all servers"
    log "Changed 'validatorPrivateKey' to 'validatorPrivateKeys'"
