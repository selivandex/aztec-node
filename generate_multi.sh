...
# Create Python script for CSV to hosts conversion with grouped wallets
...
def create_hosts_file(servers: List[Dict[str, str]], hosts_path: str, server_prefix: str, start_index: int, group_size: int = 10) -> None:
    """Create Ansible hosts file with grouped wallets."""
    try:
        with open(hosts_path, 'w') as hosts_file:
            hosts_file.write("[aztec_nodes]\n")
            
            total_groups = (len(servers) + group_size - 1) // group_size
            log(f"Grouping {len(servers)} wallets into {total_groups} groups of up to {group_size} wallets each")
            
            # Group servers by group_size (default 10)
            for i in range(0, len(servers), group_size):
                group = servers[i:i+group_size]
                group_num = (i // group_size) + start_index
                
                # Use first server's IP and address for the group
                first_server = group[0]
                eth_address_b64 = base64.b64encode(first_server['address'].encode()).decode()
                
                # Encode all private keys and join with comma
                private_keys_b64 = []
                for server in group:
                    key_b64 = base64.b64encode(server['private_key'].encode()).decode()
                    private_keys_b64.append(key_b64)
                
                validator_keys = ','.join(private_keys_b64)
                
                log(f"Group {group_num}: {len(group)} wallets (IP: {first_server['ip']})")
                
                hosts_file.write(
                    f"{server_prefix}{group_num} ansible_host={first_server['ip']} "
                    f"ansible_ssh_user=ubuntu "
                    f"server_ip={first_server['ip']} "
                    f"eth_address_b64={eth_address_b64} "
                    f"validator_private_key_b64={validator_keys}\n"
                )
                
        success(f"Hosts file created with {total_groups} grouped entries (from {len(servers)} total wallets)")
...
def main() -> int:
    ...
    try:
        ...
        log(f"Found {len(servers)} valid wallets")
...
# Main function
main() {
    log "=== Generate Multi-Wallet Hosts Script ==="
    ...
    if [ "$#" -lt 1 ] || [ "$#" -gt 3 ]; then
        ...
        error ""
        error "This script groups 10 wallets per inventory line"
        exit 1
    fi
    ...
    log "Grouping: 10 wallets per line"
    ...
    if python3 "$TEMP_CONVERTER" "$CSV_FILE" "$HOSTS_FILE" "$SERVER_PREFIX" "$START_INDEX"; then
        ...
        if [ -f "$HOSTS_FILE" ]; then
            ...
            head -3 "$HOSTS_FILE" | while IFS= read -r line; do
        ...
        success "Each line contains up to 10 grouped wallet private keys"
    else
